...
No hay tests automatizados incluidos actualmente. Puedes añadir pruebas unitarias que comprueben la carga de `config.json` y la inicialización de `AGENT_INSTRUCTIONS`.

## Siguientes pasos recomendados

- Añadir validación más estricta del `config.json` (esquema JSON).
- Añadir tests básicos (py.test) que verifiquen carga de configuración y comportamiento esperado.
- Permitir sobreescritura por variables de entorno (por ejemplo, `STT_LANGUAGE`) si se requiere despliegue sin cambiar archivos de configuración.

---

Si quieres, puedo añadir ahora:
- soporte para sobreescritura por variables de entorno,
- o un archivo `requirements.txt` y un comando de instalación alternativo.
